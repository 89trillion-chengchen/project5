...
class MemcacheHelper {
    ...
    public function __construct($config) {
         ...
         */
        if ($config) {
            $this->memcached = MemcachedManager::getInstance($config);
        }
    }
     ...
     * @return bool 
     */
    public function add($key, $value, $expiration = 0, $flag = false) {
        return $this->memcached ? $this->memcached->add($key, $value, $flag, $expiration) : false;
    }
     ...
     * @return bool 
     */
    public function replace($key, $value, $flag = false, $expiration = 0) {
        return $this->memcached ? $this->memcached->replace($key, $value, $flag, $expiration) : false;
    }
     ...
     * @return bool 
     */
    public function set($key, $value, $expiration = 0, $flag = false) {
        return $this->memcached ? $this->memcached->set($key, $value, $flag, $expiration) : false;
    }
